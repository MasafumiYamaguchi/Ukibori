#!/usr/bin/env node
// #33 real-browser WASM parity + lifecycle runner (npm run test:wasm-browser
// -w ukibori-renderer).
//
// Reproducible browser integration test (no GPU required; real Chrome):
//
...
//      bundle (+ source map) and the WASM harness (wasm-parity.html/.mjs,
//      catalog.mjs) into it
//   3. serves only that directory on 127.0.0.1 (ephemeral port) and runs
//      Chrome headless under an isolated profile
//   4. the page runs the #33 parity/lifecycle suite: WasmCpuPipeline output
//      vs the TypeScript oracle over the #30 catalog scene fixtures, stage
//      provenance, selection rules, concurrent-load dedup, load failure +
//      retry, AbortSignal cancellation, memory growth and idempotent
//      disposal
...
//      unambiguous marker and exits 0 ONLY on a real-browser PASS; FAIL and
//      SKIP (no Chrome) both exit nonzero
//
 ...
 * marker token (`UKIBORI_WASM_PASS/FAIL/SKIP`). The first line decides the
 * outcome: a FAIL detail line that happens to contain `UKIBORI_WASM_PASS`
 * later in the text must NOT flip the result. Returns the exact marker
 * token, or null when the first line carries none.
 */
export function parseWasmResultMarker(text) {
  const firstLine = String(text).split("\n", 1)[0].trim();
  const match = /^(UKIBORI_WASM_(?:PASS|FAIL|SKIP))(?:[ \t]|$)/.exec(firstLine);
  return match === null ? null : match[1];
}

async function publishResult(text) {
  const resultPath = process.env.WASM_RESULT_PATH;
...
/** SIGKILL the Chrome child and wait (bounded) for its exit. The caller
 * must not delete the browser's profile/temp directory before the process
 * has actually terminated; this never hangs when the child already exited. */
...
async function main() {
  // 1. build the public renderer ESM first (buffered so WASM_RESULT_PATH can
  //    always begin with the anchored marker even with npm banners around).
  ...
  if (!existsSync(CHROME)) {
    await publishResult(
      `${MARKER_SKIP} headless Chrome not found at ${CHROME} ` +
        "(set CHROME_PATH to the chrome binary); the browser WASM gate cannot run on this machine",
    );
    process.exit(1);
  }

  // 2. unique isolated temp directory with ONLY the bundle + harness.
  const tmp = await mkdtemp(join(tmpdir(), "ukibori-wasm-"));
  let chrome = null;
  let server = null;
  let cdp = null;
  try {
    ...
    for (const file of ["wasm-parity.html", "wasm-parity.mjs", "catalog.mjs"]) {
    ...
    // 3. serve ONLY this directory on 127.0.0.1 (basename-only lookup).
    server = createServer((req, res) => {
      const pathname = decodeURIComponent(new URL(req.url ?? "/", "http://127.0.0.1").pathname);
      const name = pathname === "/" ? "wasm-parity.html" : basename(pathname);
    ...
    // 4. isolated Chrome profile + CDP for deterministic completion.
    const cdpPort = await freePort();
    const url = `http://127.0.0.1:${port}/wasm-parity.html`;
    chrome = spawn(
      CHROME,
    ...
    while (target === null && Date.now() < targetDeadline && chrome.exitCode === null) {
      try {
        ...
        target =
          targets.find((t) => t.type === "page" && t.url.includes("wasm-parity.html")) ?? null;
      } catch {
    ...
    if (target !== null) {
      ...
      while (result === null && Date.now() < deadline && chrome.exitCode === null) {
        ...
        if (typeof text === "string" && parseWasmResultMarker(text) !== null) {
          result = text;
          break;
        }
        await sleep(200);
    ...
    if (result !== null) {
      ...
      const marker = parseWasmResultMarker(result);
      if (marker === MARKER_PASS) {
        console.log(`test:wasm-browser: ${MARKER_PASS}`);
        process.exitCode = 0;
      } else {
        // FAIL and SKIP are both failures for this gate (SKIP = Chrome
        // unavailable or the harness reported it)
        console.log(`test:wasm-browser: ${marker ?? MARKER_FAIL}`);
        process.exitCode = 1;
      }
    } else {
      await publishResult(
        `${MARKER_FAIL} harness produced no result within ${RESULT_TIMEOUT_MS}ms ` +
          `(chrome stderr tail: ${chromeStderr.slice(-500)})`,
      );
      process.exitCode = 1;
    }
  } finally {
...
// Run as a CLI only when this file is the entry module; importing the marker
// parser from Vitest must not launch Chrome or a local server.
