...
// Retain only the latest value per live element. Weak keys do not keep
// unmounted glyphs alive; null results are cached too.
const computedColorCache = new WeakMap<HTMLElement, { css: string; color: LinearRgb | null }>();
...
export function readOpaqueComputedTextColor(element: HTMLElement): LinearRgb | null {
  try {
    // Always read live CSS so inherited/theme/CSSOM invalidation keeps working.
    // Light-only frames need neither token arrays nor three transfer curves.
    const css = getComputedStyle(element).color;
    const cached = computedColorCache.get(element);
    if (cached?.css === css) return cached.color;
    const color = parseOpaqueComputedSrgb(css);
    if (color !== null) Object.freeze(color);
    computedColorCache.set(element, { css, color });
    return color;
  } catch {
    computedColorCache.delete(element);
    return null;
  }
}
...
// Number() alone also accepts empty fields and hexadecimal tokens, neither
// of which is a CSS number. Unsupported syntax must keep DOM ink visible.
