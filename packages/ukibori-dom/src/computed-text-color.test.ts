...
describe("#56 computed CSS text color", () => {
  ...
  it("reuses unchanged conversion and refreshes both color and alpha transitions", () => {
    const span = document.createElement("span");
    document.body.appendChild(span);
    span.style.color = "rgb(128, 128, 128)";
    const gray = readOpaqueComputedTextColor(span);
    expect(readOpaqueComputedTextColor(span)).toBe(gray);
    expect(Object.isFrozen(gray)).toBe(true);
    span.style.color = "rgba(128, 128, 128, 0.5)";
    expect(readOpaqueComputedTextColor(span)).toBeNull();
    expect(readOpaqueComputedTextColor(span)).toBeNull();
    span.style.color = "rgb(0, 0, 255)";
    expect(readOpaqueComputedTextColor(span)).toEqual({ r: 0, g: 0, b: 1 });
  });
});
